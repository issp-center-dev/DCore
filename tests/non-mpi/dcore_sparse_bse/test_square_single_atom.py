...
from dcore._testing import mk_hr_square, gk_tail, check_self_energy, gk_from_w90
...
def test_square(norb_inequiv_sh, spin_orbit, corr_to_inequiv, request):
    ...
    with HDFArchive('square_vertex.h5', 'r') as h:
        ...
        nfreq = G2_loc_ref.shape[0]
        G2_loc_read = float_to_complex_array(h['G2loc/sh0'])
        assert G2_loc_read.shape == (nfreq,) + (2, 1) * 4
        G2_loc_ref = G2_loc_ref.reshape(G2_loc_read.shape)
        assert_allclose(G2_loc_ref, G2_loc_read)
    ...
    chi_mat = chi[0, 0, :, :, :, :].reshape((nf,)*4).transpose((0,1,3,2)).reshape(nf**2, nf**2)
    atol = numpy.abs(chi_mat).max()
    assert_allclose(chi_mat, chi_mat.T.conj(), rtol=0, atol=1e-8*atol)
    evals, _ = numpy.linalg.eigh(chi_mat)

    assert all(evals[:3*ncorr_shell] < -5e+1) # Correspond to (pi, pi)
    assert all(abs(evals[3*ncorr_shell:]) < 1e-1) # Rest
    print(evals)
