...
class Wannier90Converter(ConverterTools):
    ...
    def convert_dft_input(self):
        ...
        print("Reading input from %s..." % self.inp_file)
        ...
        try:
            ...
            print(self._name + ": reading input file %s failed!" %
                       self.inp_file)
        ...
        n_inequiv_shells, corr_to_inequiv, inequiv_to_corr = ConverterTools.det_shell_equivalence(
            self, corr_shells)
        print("Number of inequivalent shells: %d" % n_inequiv_shells)
        print("Shell representatives: " + format(inequiv_to_corr))
        shells_map = [inequiv_to_corr[corr_to_inequiv[ish]]
                      for ish in range(n_corr_shells)]
        print("Mapping: " + format(shells_map))
        ...
        for isp in range(n_spin):
            ...
            # WFs, H(R)
            print("... done: %d R vectors, %d WFs found" % (nr, nw))
            ...
            if isp == 0:
                ...
                if self.nwfs < dim_corr_shells:
                    ...
                    print("Number of WFs larger than correlated orbitals:\n" +
        ...
        print("The k-point grid has dimensions: %d, %d, %d" % tuple(nki))
    ...
    def read_wannier90hr(self, hr_filename="wannier_hr.dat"):
        ...
        try:
            ...
            print("Wrong data or structure in file %s" % hr_filename)
    ...
    def find_rot_mat(self, n_sh, sh_lst, sh_map, ham0):
                ...
                for i in range(nw):
                    for j in range(i + 1, nw):
                        if (abs(eigval[j] - eigval[i]) < self._w90zero):
                            print("WARNING: degenerate eigenvalue of H(0) detected for shell %d: " % (ish) +
        ...
        for ish in range(n_sh):
            ...
            if not numpy.allclose(eigval_lst[ish], eigval_lst[sh_map[ish]], atol=self._w90zero, rtol=1.e-15):
                ...
                print("Eigenvalue difference: " + format(eigval_diff))
                istatus = 0
