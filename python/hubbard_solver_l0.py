# Original file: triqs_install_path/lib/python2.7/site-packages/pytriqs/applications/impurity_solvers/hubbard_I/hubbard_solver.py
...
from pytriqs.applications.impurity_solvers.hubbard_I.hubbard_solver import gf_hi_fullu, sigma_atomic_fullu
...
#            cnt += 1
    ...
    def __set_umatrix(self,U,J,T=None):
        ...
        if self.l == 0:
            Fk = np.array([U,])
            Umat = U_matrix(l=self.l, radial_integrals=Fk, T=T)
        else:
            Umat = U_matrix(l=self.l, U_int=U, J_hund=J, T=T)
        U, Up = reduce_4index_to_2index(Umat)
